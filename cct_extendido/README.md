...
| **Regla ET (tiempo)**          | (la parametrización) `school_share_ET`, `att_min`, `transfer_rule` | **Real (norma de asistencia)** + calibración | Muchos CCT exigen ≥85% asistencia. Si ET implica asistencia parcial, define si el pago es **proporcional** o **todo/nada** (umbral). Basado en reglas del programa.                                                                                           |
